...
print(brail_sample_org_data_list)
brail_number_of_classes = len(brail_sample_org_data_list)
print(brail_number_of_classes)
...
'''
model = Sequential()
model.add(Conv2D(noOfFilters, sizeOfFilter1, activation='relu', input_shape=(resize_shape[0], resize_shape[1], 1))) # layer 01
model.add(MaxPooling2D(pool_size=sizeOfPool, strides=2))
...
model.add(Conv2D(noOfFilters, sizeOfFilter1, activation='relu', input_shape=(resize_shape[0], resize_shape[1], 1)))
model.add(MaxPooling2D(pool_size=sizeOfPool, strides=2))
...
history = model.fit(x_train,y_train,validation_data=(x_validation,y_validation), epochs=500,verbose=1)
...
# input
input_image = cv2.imread("TestingImages/3.jpg",cv2.IMREAD_COLOR)
...
# plt.imshow(input_image, cmap='gray')
# plt.show()
# print("predicted class33: ",predicted_class)
